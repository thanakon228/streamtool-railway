...
function createSongRequest({ apiKey, persistence, emitState, emitNowPlaying }) {
  ...
  function setPaused(on) { state.paused = !!on; emitState(pub()); }
  ...
  function captureChat(msg) {
    ...
    // Command must be at the START of the chat message (avoids "อยากขอเพลงนี้จัง" matching)
    if (!text.toLowerCase().startsWith(state.keyword.toLowerCase())) return;
    const raw = text.slice(state.keyword.length).replace(/^[\s:：>\-]+/, "").trim();
    if (!raw) return;
